...
io.on('connection', function(socket){
	...
	/*--------------------------------------
	       Process Success Additions
	---------------------------------------*/	
	socket.on('msgprint', function(msg){
		console.log('message: ' + msg);
		io.emit('msgprint', msg);
	});

	socket.on('update_item', function(args){
		var doctype=args.doctype;
		var name = args.item.name;
		// console.log("________WMMIT UPDATE CAUGHT ______"  + name);
		// console.log('update_'+doctype+'_'+name);
		// console.log(args);
		io.emit('update_'+doctype+'_'+name , args.item);
	});
	socket.on('update_doc', function(args){
		var doctype=args.doctype;
		var name = args.item.name;
		// console.log("________WMMIT UPDATE CAUGHT ______"  + name);
		// console.log('update_'+doctype+'_'+name);
		// console.log(args);
		io.emit('update_'+doctype, args.item);
	});
	socket.on('create_doc', function(args){
		var name = args.item.name;
		var doctype = args.doctype;

		io.emit('create_'+doctype , args.item);
	});
	socket.on('remove_doc', function(args){

		var doctype = args.doctype;
		console.log(args);
		console.log("________WMMIT REMOVE CAUGHT ______");
		console.log("________sending: create_" +doctype);
		io.emit('remove_'+ doctype , args.name);
	});
	//io.emit('update_'+doctype, args.item);
